...
def handle_output(frame, result, init_w,init_h,prob_threshold):
    ...
    for i, p in enumerate(probs):
        if p > prob_threshold:
            current_count += 1
            box = result[0, 0, i, 3:]
            p1 = (int(box[0] * init_w), int(box[1] * init_h))
            p2 = (int(box[2] * init_w), int(box[3] * init_h))
...
def infer_on_stream(args, client):
   ...
   # n, c, h, w = infer_network.load_model(args.model, args.device,current_req, args.cpu_extension)[1]
    infer_network.load_model(args.model, args.device,current_req, args.cpu_extension)

    net_shapes = infer_network.get_inner_input_shapes()

    n,c,h,w = net_shapes['image_tensor']
    ...
    while cap.isOpened():
        ...
        '''
        preprocessed_frame = preprocess_frame(frame,n,c,h,w)

        print ('preprocessedframe',preprocessed_frame.shape[1:])

        image_info = [h,w,1]

        net_input = {'image_tensor': preprocessed_frame,'image_info': [n,image_info]}
        '''
        ...
        ### Start asynchronous inference for specified request ###
        #infer_network.exec_net(current_req, preprocessed_frame)
        infer_network.exec_net(current_req, net_input)
